...
if __name__ == "__main__":
    # Default dates: from 5 days ago to yesterday
    end_date_default = datetime.today() - timedelta(days=1)
    start_date_default = end_date_default - timedelta(days=5)

    parser = argparse.ArgumentParser(description='Download stock data from Yahoo Finance.')
    parser.add_argument('ticker', type=str, help='Stock ticker symbol')
    parser.add_argument('--start_date', type=str, default=start_date_default.strftime('%Y-%m-%d'), help='Start date in the format YYYY-MM-DD (default: 5 days ago)')
